...
const render = () => {
  ...
  if (props.data.length > 0) {
    // Use x,y coordinates for scatter plot, fallback to timestamp/value
    const points = props.data.map(point => ({
      x: point.x ? (point.x / 1000) * width : (point.timestamp % 1000) / 1000 * width,
      y: point.y ? (point.y / 1000) * height : (point.value / 100) * height
    }));

    // Draw points with different colors based on category
    const colors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b'];
    
    // Only render first 1000 points for performance
    const displayPoints = points.slice(0, 1000);
    
    displayPoints.forEach((point, index) => {
      const color = colors[index % colors.length];
      renderer!.drawPoints([point], color, 4);
    });
  }
};
...
onMounted(() => {
  if (canvasRef.value) {
    try {
      ...
      console.log('✅ ScatterPlot mounted successfully');
    } catch (error) {
      console.error('❌ ScatterPlot mount error:', error);
    }
  }
});
